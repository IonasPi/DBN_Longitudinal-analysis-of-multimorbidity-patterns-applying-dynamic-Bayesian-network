...
This R Markdown is divided in four sections:

# 1. Exploratory data analysis and data preprocessing

# 2. Building the time series dataset
## 2.1 Incidence

# 3. Dynamic Bayesian network analysis
## 3.1 DMMHC algorithm
## 3.2 PSOHO algorithm
## 3.3 natPSOHO algorithm

# 4. Other: Gantt chart and table 2

# References


Sections #1 and #2 explain the steps to preprocess the data and build the time series that I did in this project.
Using the cumulative incidence table that can be obtained from the associated pdf document, you can skip sections 1 and 2 and go directly to performing the dynamic Bayesian network analysis in section #3. 


Loading the data
```{r setup, include=FALSE}
# Diagnostic registries of patients with chronic conditions and fulfill the inclusion criteria
dat <-  read.delim("C:/X/datos")

# Patients that fulfill the inclusion criteria
dat2 <- read.delim("C:/X/datos_estudio_todos")
...
```{r setup, include=FALSE}
str(dff)
u1<- as.data.frame(unique(dat$COD_N1)) 
u2<- as.data.frame(unique(dat2$COD_N2)) 
```
There are a total of 872,649 registries. 184,647 unique patients with chronic conditions. 246,144 patients in total. 


A general exploratory data analysis is performed. 
```{r setup, include=FALSE}
...
Apparently there are no missing values, although they are -1 o -2 in some cases, which correspond to real missing values. I have to check in detail these cases considering the specific codification in the original database.  

I create the variable "death_date", I have to consider COD_FEC_FALLECIMIENTO, COD_FEC_BAJA_BDU and COD_MOTIVO_BAJA. In most cases COD_FEC_FALLECIMIENTO includes the right information but sometimes I have to consider COD_FEC_BAJA_BDU when the variable COD_MOTIVO_BAJA = 1 (DEATH).   
```{r setup, include=FALSE}

dat$death_date <- ifelse(dat$COD_FEC_FALLECIMIENTO > 0, dat$COD_FEC_FALLECIMIENTO, ifelse(dat$COD_FEC_FALLECIMIENTO < 1 & (dat$COD_FEC_BAJA_BDU > 1 & dat$COD_MOTIVO_BAJA == 1), dat$COD_FEC_BAJA_BDU, -1))

dat2$death_date <- ifelse(dat2$COD_FEC_FALLECIMIENTO > 0, dat2$COD_FEC_FALLECIMIENTO, ifelse(dat2$COD_FEC_FALLECIMIENTO < 1 & (dat2$COD_FEC_BAJA_BDU > 1 & dat2$COD_MOTIVO_BAJA == 1), dat2$COD_FEC_BAJA_BDU, -1))
...
I analyze the variables: chronic diseases, sex and initial date when diagnostics are done. They are key variables and help to clean the dataset.
```{r setup, include=FALSE}
# Chronic diseases
u3<- unique(dff$DESC_PATOLOGIA_CRONICA) # 80 chronic conditions
table(dff$COD_PATOLOGIA_BPS)
#plot(dff$COD_PATOLOGIA_BPS)

# Sex
table(dff$COD_SEXO)
df<- subset(dff, dff$COD_SEXO == 0 | dff$COD_SEXO == 1) # I keep the registry if I know the sex
dff <- df
rm(df)
df<- subset(dff2, dff2$COD_SEXO == 0 | dff2$COD_SEXO == 1) # I keep the registry if I know the sex
dff2 <- df
rm(df)
u1<- as.data.frame(unique(dff$COD_N1)) # 184,568 unique patients with chronic conditions
u2<- as.data.frame(unique(dff2$COD_N2)) # 243,792 patients in total.

# Description of the dates when diagnostics are done 
dff <- transform(dff, FECHA_INICIO_PAT  = as.Date(as.character(FECHA_INICIO_PAT), "%Y%m%d"))
dff <- transform(dff, COD_FEC_NACIMIENTO  = as.Date(as.character(COD_FEC_NACIMIENTO), "%Y%m%d"))
dff$diff_in_years <- difftime(dff$FECHA_INICIO_PAT, dff$COD_FEC_NACIMIENTO, units = "weeks")/52.25

u4<- unique(dff$diff_in_years)   
plot(dff$diff_in_years)
summary(dff$diff_in_years)
table(dff$diff_in_years)  
plot(table(dff$diff_in_years)) # I plot the distribution. It shows that mostly the diagnoses are done in the range 40-65 years old 

df<- subset(dff, dff$diff_in_years >= 0   & dff$diff_in_years < 66) # I keep the registry if it was done before 66 years old to follow up the population 45-65 years old. 
dff <- df
rm(df)
u1<- as.data.frame(unique(dff$COD_N1)) # 182,071 unique ID with chronic conditions 
```
I keep the registry if it was done before 66 years old. There are 182,071 unique ID with chronic conditions. 243,792 patients in total. 


The active time in the system is another variable important to understand and filter the data. Some patients could be active for a few days and some others for the whole time. Regarding dead people, I have to consider the date of death. 
A minimum time is set to consider a patient to be included in the study.
```{r setup, include=FALSE}

summary(dff$NUM_TIEMPO_EN_ACTIVO) # As I saw previously there are no missing values as NA data, in this case it is a -1 value.

df<- subset(dff, dff$NUM_TIEMPO_EN_ACTIVO >= 0) # I keep the registry if active time is >= 0 days to remove missing data
dff <- df
rm(df)
df<- subset(dff2, dff2$NUM_TIEMPO_EN_ACTIVO >= 0) # I keep the registry if active time is >= 0 days to remove missing data
dff2 <- df
rm(df)
u1<- as.data.frame(unique(dff$COD_N1)) # 180,868 unique ID with chronic conditions
u2<- as.data.frame(unique(dff2$COD_N2)) # 228,776 patients in total.


date1 <-  as.Date("2001-01-01")
date2 <-  as.Date("2021-12-31")
dff <- transform(dff, death_date  = as.Date(as.character(death_date), "%Y%m%d"))
dff$porc_activo_sist_mort <- dff$NUM_TIEMPO_EN_ACTIVO/(as.numeric(difftime(dff$death_date, date1, units = "days")))
dff["porc_activo_sist_mort"][is.na(dff["porc_activo_sist_mort"])] <- 0
dff$porc_activo_sist <- dff$NUM_TIEMPO_EN_ACTIVO/(as.numeric(difftime(date2, date1, units = "days")))
dff$porc_activo <- ifelse(dff$porc_activo_sist_mort > dff$porc_activo_sist, dff$porc_activo_sist_mort, dff$porc_activo_sist)
summary(dff$porc_activo) # Mean 0.90, Median 0.97
dff <- subset(dff, dff$porc_activo_sist >= 0.99) # I keep those with active time >= 99% of the total possible time.
u1<- as.data.frame(unique(dff$COD_N1)) # 124,459 unique ID with chronic conditions


dff2 <- transform(dff2, death_date  = as.Date(as.character(death_date), "%Y%m%d"))
dff2$porc_activo_sist_mort <- dff2$NUM_TIEMPO_EN_ACTIVO/(as.numeric(difftime(dff2$death_date, date1, units = "days")))
dff2["porc_activo_sist_mort"][is.na(dff2["porc_activo_sist_mort"])] <- 0
dff2$porc_activo_sist <- dff2$NUM_TIEMPO_EN_ACTIVO/(as.numeric(difftime(date2, date1, units = "days")))
dff2$porc_activo <- ifelse(dff2$porc_activo_sist_mort > dff2$porc_activo_sist, dff2$porc_activo_sist_mort, dff2$porc_activo_sist)
summary(dff2$porc_activo) # Mean 0.88, Median 0.97
dff2 <- subset(dff2, dff2$porc_activo_sist >= 0.99) # I keep those with active time >= 99% of the total possible time.
u2<- as.data.frame(unique(dff2$COD_N2)) # 131,574 patients in total 

# To clean the environment I remove the dates
rm(date1, date2, u)

```


The population is described by sex
```{r setup, include=FALSE}
# Men
dff_men <- subset(dff, dff$COD_SEXO == 0) #63,258 men with chronic conditions
u3 <- as.data.frame(unique(dff_men$COD_N1))
dff_men2 <- subset(dff2, dff2$COD_SEXO == 0) #67,713 men in total
u4 <- as.data.frame(unique(dff_men2$COD_N1))

# Women
dff_wom <- subset(dff, dff$COD_SEXO == 1) #61,201 women with chronic conditions
u5 <- as.data.frame(unique(dff_wom$COD_N1))
dff_wom2 <- subset(dff2, dff2$COD_SEXO == 1) #63,861 women in total 
u6 <- as.data.frame(unique(dff_wom2$COD_N1))

# How many different diseases are included in the study
# Total
dff_enf <- as.data.frame(unique(dff$COD_PATOLOGIA_BPS))
dff_enf[,2]<- 0
rownames(dff_enf) <- dff_enf[,1]
dff_enf<-dff_enf[-1]
dff_enf<- as.data.frame(t(dff_enf)) # I keep them because they will be useful later on. 80 conditions in total  

rm(dff_men, dff_men2, dff_wom, dff_wom2)
```


# 2. Building the time series dataset
I need to study the population by four-month periods to be able to create the time series planned (cumulative incidences in each period) 
```{r setup, include=FALSE}
# To calculate the four-month periods, first, I get difference between date of diagnostic to date of birth when they are 45 years old.
...
# I subset the population to follow them 
dffi <- subset(dff, dff$cuat >= 0 & dff$cuat < 63)

# To calculate the number of deaths in each four-month period I analyze the whole set of patients in dff2. 
...
dff21<- subset(dff2, dff2$cuatmort >= 0 & dff2$cuatmort < 63) 

``` 


I create a subset for each 4-month period
```{r setup, include=FALSE}
...
# I get the number of deaths for each period
...
Now, I can create the matrix ID/BPS-codes for each subset
```{r setup, include=FALSE}
library(qdap)
mif<- function(x) return(mtabulate(split(x[,3],x[,1])))
dat_mati<- lapply(dat_subseti,mif)

# For dead people. I get the matrix of diseases in each period
# First, I have to get the dead patient who had chronic conditions. 
dff <- transform(dff, COD_FEC_NACIMIENTO  = as.Date(as.character(COD_FEC_NACIMIENTO), "%Y%m%d"))
...
dff22<- subset(dff, dff$cuatmort >= 0 & dff$cuatmort < 63) 
dat_subset_mort2 <- split(dff22, dff22$cuatmort)
# Second, I apply the function mif to get the diseases.
dat_mat_mort2<- lapply(dat_subset_mort2,mif)

```

As I have the first diagnostic date for each patient and group according to this time and diagnosis, I should have a matrix with binary information (0s, 1s). I check it and is correct.  
...
w0<- x-n0 #Total at the beginning - incidents previously - deaths previously
...
dat43inmort <- datin # A patient died in t43 but he/she did not have any chronic condition
...
z43<- dat_mat_mort$`43` #I consider here that a patient died in t43
y43<- y42-z43         
...
dat52inmort <- datin # A patient died in t52 but he/she did not have any chronic condition
...
z52<- dat_mat_mort$`52` # A patient died in t52 but he/she did not have any chronic condition
y52<- y51-z52        
...
dat60in<- t(as.data.frame(colSums(dat_mati[["60"]])))
dat60in<- merge(dat60in, datin, all=T)
dat60in<- t(as.data.frame(colSums(dat60in)))
dat60in[is.na(dat60in)] <- 0

z59<- dat_mat_mort$`59`
y59<- y58-z59         
n59<- t(as.data.frame(colSums(merge (dat59in, dat59inmort, all=T))))
w59<- y58-n59  
dat60inc <- merge(dat60in, w59, all= T)
dat60inc <- (dat60inc[1,]/dat60inc[2,])*100

dat60mort<- t(as.data.frame(colSums(dat_mat_mort2$`60`)))
dat60mort<- -1*dat60mort
datin <- dff_enf 
dat60inmort <- merge(dat60mort, datin, all=T)
dat60inmort <- t(as.data.frame(colSums(dat60inmort)))
dat60inmort[is.na(dat60inmort)] <- 0
dat60inmort <- as.data.frame(dat60inmort)

dat61in<- t(as.data.frame(colSums(dat_mati[["61"]])))
dat61in<- merge(dat61in, datin, all=T)
dat61in<- t(as.data.frame(colSums(dat61in)))
dat61in[is.na(dat61in)] <- 0

z60<- dat_mat_mort$`60`
y60<- y59-z60         
n60<- t(as.data.frame(colSums(merge (dat60in, dat60inmort, all=T))))
w60<- y59-n60 
dat61inc <- merge(dat61in, w60, all= T)
dat61inc <- (dat61inc[1,]/dat61inc[2,])*100

dat61mort<- t(as.data.frame(colSums(dat_mat_mort2$`61`)))
dat61mort<- -1*dat61mort
datin <- dff_enf 
dat61inmort <- merge(dat61mort, datin, all=T)
dat61inmort <- t(as.data.frame(colSums(dat61inmort)))
dat61inmort[is.na(dat61inmort)] <- 0
dat61inmort <- as.data.frame(dat61inmort)


dat62in<- t(as.data.frame(colSums(dat_mati[["62"]])))
dat62in<- merge(dat62in, datin, all=T)
dat62in<- t(as.data.frame(colSums(dat62in)))
dat62in[is.na(dat62in)] <- 0

z61<- dat_mat_mort$`61`
y61<- y60-z61         
n61<- t(as.data.frame(colSums(merge (dat61in, dat61inmort, all=T))))
w61<- y60-n61  
dat62inc <- merge(dat62in, w61, all= T)
dat62inc <- (dat62inc[1,]/dat62inc[2,])*100
...
plot(datosinc$`914`) # HF. Increase slowly with peak at the end
plot(datosinc$`910`) # IHD. Increase slowly with peak at the end
plot(datosinc$`402`) # DLP. No flat phase, ascend and descend. Peak around t30 (55 years old)
plot(datosinc$`401`) # DM2. Ascend, peak around t30 and stable or slow-descend from then
plot(datosinc$`403`) # Hypothiroidism. Increase until t30 and then stability with some decrease
plot(datosinc$`404`) # Obesity. Increase slowly, possible peak around t40, flat phase at the end
plot(datosinc$`1001`) # COPD. Increase slowly, possible peak around t40, flat phase at the end
plot(datosinc$`1002`) # Asthma. Flat phase, and decrease slowly
plot(datosinc$`1302`) # Arthrosis. Increase until t30 and then stability with some decrease
plot(datosinc$`507`) # Anxiety. Small flat phase (t20-t30), ascend and descend. Peak around t20 (52 years old)
plot(datosinc$`506`) # Depression. Increase until t40 and then stability with some decrease
plot(datosinc$`505`) # Psychosis. No clear pattern. It seems that is very random and tends to decrease with time
plot(datosinc$`1401`) # Renal failure. Increase slowly with peak at the end
plot(datosinc$`500`) # Dementia. Increase slowly with peak at the end  
plot(datosinc$`1202`) # Atopic dermatitis. Slowly ascend with high range of values
plot(datosinc$`1201`) # Psoriasis. Slowly ascend with high range of values, peak around t30, and then flat phase
plot(datosinc$`1305`) # Arthritis. Slowly ascend with high range of values
plot(datosinc$`502`) # Alcohol dependence. Increase slowly with peak at the end 
plot(datosinc$`503`) # Tobacco dependence. Fast increase, peak inc t20 (52 years old), slowly descend. 
plot(datosinc$`504`) # Other substance abuse. No clear pattern. It seems that is very random and tends to increase with time 
plot(datosinc$`1101`) # Chronic liver disease (not cirrhosis). Increase with a peak at the end. From t40 range of values increase 
plot(datosinc$`101`) # HIV. Slow descend with high range of values. 
plot(datosinc$`210`) # Breast cancer. Increase with peak at the end. High range inc close times.
plot(datosinc$`203`) # Colon cancer. Increase with peak at the end 
plot(datosinc$`206`) # Lung cancer. Flat phase until t45, and rapid ascend

merged_data3<- datosinc
...
#Plot to show total # of incident cases per four-month period. 
datosin$suma <- rowSums(datosin) 
plot(datosin$suma)  # Total. Increase until t30 and then stability with some decrease
# Most prevalent and relevant chronic conditions
plot(datosin$`913`) # HTA. Similar pattern with acuter decrease
plot(datosin$`914`) # HF. Increase slowly with peak at the end
plot(datosin$`910`) # IHD. Increase slowly with peak at the end
plot(datosin$`402`) # DLP. No flat phase, ascend and descend. Peak around t30 (55 years old)
plot(datosin$`401`) # DM2. Ascend, peak around t30 and stable or slow-descend from then
plot(datosin$`403`) # Hypothiroidism. Increase until t30 and then stability with some decrease
plot(datosin$`404`) # Obesity. Increase slowly, possible peak around t40, flat phase at the end
plot(datosin$`1001`) # COPD. Increase slowly, possible peak around t40, flat phase at the end
plot(datosin$`1002`) # Asthma. Flat phase, and decrease slowly
plot(datosin$`1302`) # Arthrosis. Increase until t30 and then stability with some decrease
plot(datosin$`507`) # Anxiety. Small flat phase (t20-t30), ascend and descend. Peak around t20 (52 years old)
plot(datosin$`506`) # Depression. Increase until t40 and then stability with some decrease
plot(datosin$`505`) # Psychosis. No clear pattern. It seems that is very random and tends to decrease with time
plot(datosin$`1401`) # Renal failure. Increase slowly with peak at the end
plot(datosin$`500`) # Dementia. Increase slowly with peak at the end  
plot(datosin$`1202`) # Atopic dermatitis. Slowly ascend with high range of values
plot(datosin$`1201`) # Psoriasis. Slowly ascend with high range of values, peak around t30, and then flat phase
plot(datosin$`1305`) # Arthritis. Slowly ascend with high range of values
plot(datosin$`502`) # Alcohol dependence. Increase slowly with peak at the end 
plot(datosin$`503`) # Tobacco dependence. Fast increase, peak in t20 (52 years old), slowly descend. 
plot(datosin$`504`) # Other substance abuse. No clear pattern. It seems that is very random and tends to increase with time 
plot(datosin$`1101`) # Chronic liver disease (not cirrhosis). Increase with a peak at the end. From t40 range of values increase 
plot(datosin$`101`) # HIV. Slow descend with high range of values. 
plot(datosin$`210`) # Breast cancer. Increase with peak at the end. High range in close times.
plot(datosin$`203`) # Colon cancer. Increase with peak at the end 
plot(datosin$`206`) # Lung cancer. Flat phase until t45, and rapid ascend 
...
Column names are translated to chronic conditions in English 
...
write.csv(merged_data3, "C:/X\\merged_data3", row.names=T)
```


To clean the environment and facilitate the computation
```{r structure, include=FALSE}
rm(dat_cont,dat_cont_men,dat_cont_men2,dat_cont_wom,dat_cont_wom2,dat_cont2, dat_mat, dat_mat2, dat_mat2_men, dat_mat2_wom, dat_mat_men, dat_mat_wom, dat_subset, dat_subset_men, dat_subset_wom, data, data_men, data_wom, dff, dff2, dff3, dff4, DBNdatatsv) 
...
Once I have the cumulative incidence dataframe (merged_data3 in this script), it is possible to start the DBN analysis. 


I install the library "dbnR" and its dependencies. 
```{r structure, include=FALSE}
...
library(readxl)
gantdiagram <- read_excel("X/gantdiagram.xlsx")

ganttrify(gantdiagram,project_start_date = "2022-09")
```
...
dif <- prop.test(x = c(67713, 63861), n = c(131574, 131574))
dif1 <- prop.test(x = c(63258, 61201), n = c(67713, 63861))
dif2 <- prop.test(x = c(1135, 420), n = c(67713, 63861))


# Effect size
m <- matrix(c(67713,131574, 63861, 131574), nrow=2)
phi(m)

m1 <- matrix(c(63258,67713, 61201, 63861), nrow=2)
phi(m1)

m2 <- matrix(c(1135,67713, 420, 63861), nrow=2)
phi(m2)
...
39. Tsamardinos, I., Brown, L.E., Constantin, F., Aliferis, C.F.: The max-min hill-climbing bayesian network structure learning algorithm. Mach. Learn. 65(1), 31–78 (2006)
