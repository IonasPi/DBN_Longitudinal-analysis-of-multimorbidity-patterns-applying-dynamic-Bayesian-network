...
Sections #1 and #2 explain the steps to preprocess the data and build the time series analyzed in this project.
The time series can be obtained from the associated pdf document of this project; With this table, you can skip sections 1 and 2, and go directly to perform the dynamic Bayesian network analysis in section #3.
...
plot(table(dff$diff_in_years)) # I plot the distribution. It shows that most of the diagnoses are done in the range 40-65 years old 
...
plot(datosinc$`914`) # HF. Slow ascent with peak at the end
plot(datosinc$`910`) # IHD. Slow ascent with peak at the end
plot(datosinc$`402`) # DLP. No flat phase, rise and fall. Peak around t30 (55 years)
...
plot(datosinc$`203`) # Colon cancer. Rise with peak at the end
...
I translate column names to chronic conditions in English 
...
Once I have the cumulative incidence dataframe (merged_data3 in this script) I start the DBN analysis. If you already have the times series you can start at this point. 
...
library(writexl)
write_xlsx(arcst_dmmhcin20.2,"C:/Jonás/DBN\\arcst_dmmhcin20.2.xlsx") # I keep the model to perform the cluster analysis in Gephi. 
write_xlsx(arcs_dmmhc20df_in,"C:/Jonás/DBN\\arcs_dmmhc20df_in.xlsx") # I keep the model to perform the cluster analysis in Gephi. 
```
